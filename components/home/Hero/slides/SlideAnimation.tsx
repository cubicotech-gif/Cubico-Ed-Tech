  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', backgroundColor: '#030308' }}>
      ...
      <div style={{ height: 28, flexShrink: 0, backgroundColor: '#060610', borderBottom: '1px solid #1a1a2e', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 10px' }}>
        <span style={{ fontFamily: 'var(--font-display)', fontSize: 10, fontWeight: 700, color: '#F0EBE3' }}>Cubico Animation Studio</span>
        {/* Playback controls */}
        <div style={{ display: 'flex', gap: 8 }}>
          {['⏮', '▶', '⏭'].map((icon, i) => (
            <span key={i} style={{ fontSize: 8, color: i === 1 ? '#E8622A' : '#3a3a3a', cursor: 'pointer' }}>{icon}</span>
          ))}
        </div>
      </div>
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        ...
        <div style={{ flex: '0 0 65%', position: 'relative', backgroundColor: '#000', borderRight: '1px solid #1a1a2e', overflow: 'hidden' }}>
          ...
          <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, #030318 0%, #050a20 40%, #0a0508 100%)' }}>
            ...
            <div style={{ position: 'absolute', bottom: 8, left: 10, right: 10, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <span style={{ fontFamily: 'var(--font-accent)', fontSize: 8, color: '#C9A96E', letterSpacing: '0.15em' }}>PROPHET IBRAHIM SERIES · EP. 3</span>
              {/* Play button */}
              <div style={{ width: 22, height: 22, borderRadius: '50%', backgroundColor: 'rgba(232,98,42,0.85)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <div style={{ flex: '0 0 35%', backgroundColor: '#0a0a0a', padding: '8px 0', overflow: 'hidden' }}>
          <div style={{ fontFamily: 'var(--font-accent)', fontSize: 7, color: '#333', letterSpacing: '0.25em', padding: '0 8px 6px' }}>ASSETS</div>
          {SCENES.map((scene) => (
            <div key={scene.num} style={{ borderLeft: `2px solid ${scene.active ? '#E8622A' : 'transparent'}`, backgroundColor: scene.active ? '#141414' : 'transparent', marginBottom: 2 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 8px' }}>
                <div style={{ width: 28, height: 18, backgroundColor: scene.active ? '#1a0a05' : '#0f0f0f', borderRadius: 2, border: `1px solid ${scene.active ? '#E8622A33' : '#1a1a1a'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  <span style={{ fontFamily: 'var(--font-accent)', fontSize: 7, color: scene.active ? '#E8622A' : '#333' }}>{scene.num}</span>
                </div>
      ...
      <div style={{ height: 24, flexShrink: 0, backgroundColor: '#060606', borderTop: '1px solid #1a1a2e', display: 'flex', alignItems: 'center', gap: 8, padding: '0 8px' }}>
        <span style={{ fontFamily: 'var(--font-accent)', fontSize: 7, color: '#3a3a3a' }}>00:00</span>
        <div style={{ flex: 1, height: 3, backgroundColor: '#1a1a1a', borderRadius: 2, position: 'relative' }}>
          <div style={{ width: '45%', height: '100%', backgroundColor: '#E8622A', borderRadius: 2 }} />
          <div style={{ position: 'absolute', left: '45%', top: '50%', transform: 'translate(-50%, -50%)', width: 7, height: 7, borderRadius: '50%', backgroundColor: '#E8622A', border: '1px solid #050505' }} />
        </div>
        <span style={{ fontFamily: 'var(--font-accent)', fontSize: 7, color: '#3a3a3a' }}>02:34</span>
      </div>
    </div>
  );
});
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', backgroundColor: '#030308' }}>
      ...
      <div style={{ backgroundColor: '#0a0a0a', borderTop: '1px solid #1a1a2e', padding: '8px 10px' }}>
        <div style={{ fontFamily: 'var(--font-accent)', fontSize: 8, color: '#C9A96E', letterSpacing: '0.12em', marginBottom: 4 }}>EP. 3 · PROPHET IBRAHIM SERIES</div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <span style={{ fontSize: 10, color: '#3a3a3a' }}>⏮</span>
          <div style={{ width: 22, height: 22, borderRadius: '50%', backgroundColor: '#E8622A', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <div style={{ width: 0, height: 0, borderStyle: 'solid', borderWidth: '5px 0 5px 8px', borderColor: 'transparent transparent transparent #fff', marginLeft: 2 }} />
          </div>
          <span style={{ fontSize: 10, color: '#3a3a3a' }}>⏭</span>
          <div style={{ flex: 1, height: 2, backgroundColor: '#1a1a1a', borderRadius: 1, marginLeft: 4 }}>
            <div style={{ width: '45%', height: '100%', backgroundColor: '#E8622A', borderRadius: 1 }} />
          </div>
        </div>
      </div>
    </div>
